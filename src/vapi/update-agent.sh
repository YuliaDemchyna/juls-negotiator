...
# Update existing Vapi assistant with enhanced configuration
...
SERVER_URL="https://9082bc896ff7.ngrok-free.app"

curl -X PATCH "https://api.vapi.ai/assistant/${ASSISTANT_ID}" \
  -H "Authorization: Bearer ${VAPI_API_KEY}" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "Juls Debt Negotiation Agent",
    "voice": {
      "provider": "11labs",
    ...
    "functions": [
      {
        ...
        "parameters": {
          ...
          "required": ["phone_number"]
              ...
              "serverUrl": "'"${SERVER_URL}"'/api/vapi/get-user-info"
      },
      {
        "name": "negotiatePayment",
        "description": "Calculate negotiation response based on user'\''s payment offer",
        "parameters": {
          "type": "object",
          "properties": {
            "user_amounts": {
        ...
        "serverUrl": "'"${SERVER_URL}"'/api/vapi/negotiate"
      },
      {
        "name": "saveCallResult",
        ...
        "serverUrl": "'"${SERVER_URL}"'/api/vapi/save-result"
      }
    ],
    "analysisPlan": {
      "summaryPrompt": "Summarize the call including the initial offer, negotiation process, and final outcome.",
      ...
      "successEvaluationPrompt": "Evaluate if the agent successfully negotiated a higher payment",
